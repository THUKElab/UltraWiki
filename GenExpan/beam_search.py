...
if __name__ == "__main__":
    ...
    from utils import convert_to_tokens_ids, clear_eos_and_leading_space
    import json
    import os
    ...
    model = AutoModelForCausalLM.from_pretrained(model_path).bfloat16()
    model.cuda(7)
    # model_state_dict=torch.load('./model/epoch_5.pth',map_location=model.device)
    ...
    ents = convert_to_tokens_ids(ents, tokenizer, eos_token)
    entity_trie = EntityTrie(ents)

    prompt = "The most developed city in the United States is"
    prompt = "oil, natural gas, coal and uranium.\ngold, silver, platinum and palladium.\nCurium, Americium, Einsteinium and"
    ...
    print("output tokens:")
    for sent in sents:
        sent = clear_eos_and_leading_space(sent, eos_token)
        sent = sent.strip()
        print(sent)
    print()
    print("the probability of each generated tokens: ")
    print(res.prod_probs)
